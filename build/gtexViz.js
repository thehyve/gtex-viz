...
function parseSingleTissueEqtls(data, tissueSiteTable = undefined) {
...
function parseTissueSampleCounts(json) {
  const attr = 'tissueSummary';
  if (!json.hasOwnProperty(attr)) throw 'Parsing Error: required json attr is missing: ' + attr;
  const tissues = json[attr]; // check json structure
...
function parseExonsToList(json) {
  const attr = 'exon';
  if (!json.hasOwnProperty(attr)) throw 'Parsing Error: required json attribute is missing: exon';
  return json[attr];
}
...
function parseGeneExpressionForBoxplot(data, tissues = undefined, colors = undefined) {
  const attr = 'geneExpression';
...
function generateShortVariantId(id) {
  ...
  if (temp[2].length > temp[3].length) {
    ...
    // temp[3].length == temp[2].length and temp[3].length > 1
    temp[3] = "sub";
    temp.splice(2, 1); // delete the ref
  }
...
class Boxplot {
  ...
  render(rootId, plotOptions = {}) {
    ...
    let yAxis = axisLeft(scales.y); // render x-axis

    dom.append('g').attr('class', 'boxplot-x-axis').attr('transform', `translate(${margins.left + scales.x.bandwidth() / 2}, ${height - margins.bottom})`).call(xAxis).attr('text-anchor', 'start').selectAll('text').attr('transform', 'translate(5,1) rotate(45)').attr('font-size', xAxisFontSize); // x-axis label

    dom.append('text').attr('transform', `translate(${margins.left + width / 2 + scales.x.bandwidth() / 2}, ${height - xAxisLabelFontSize / 2})`).attr('text-anchor', 'middle').style('font-size', xAxisLabelFontSize).text(xAxisLabel); // render y-axis

    dom.append('g').attr('class', 'boxplot-y-axis').attr('transform', `translate(${margins.left}, ${margins.top})`).call(yAxis).attr('font-size', yAxisFontSize); // y-axis label

    dom.append('text').attr('transform', `translate(${yAxisLabelFontSize}, ${(height - margins.bottom) / 2}) rotate(270)`).attr('text-anchor', 'middle').style('font-size', yAxisLabelFontSize).text(yAxisLabel); // render IQR box

    dom.append('g').attr('class', 'boxplot-iqr').attr('transform', `translate(${margins.left + scales.x.bandwidth()}, ${margins.top})`).selectAll('rect').data(this.boxplotData).enter().append('rect').attr('x', d => scales.x(d.label) - scales.x.bandwidth() / 2).attr('y', d => this.useLog ? scales.y(d.q3 + adjust) : scales.y(d.q3)).attr('width', d => scales.x.bandwidth()).attr('height', d => this.useLog ? scales.y(d.q1 + adjust) - scales.y(d.q3 + adjust) : scales.y(d.q1) - scales.y(d.q3)).attr('fill', d => `#${d.color}`).attr('stroke', '#aaa').on('mouseover', (d, i, nodes) => {
    ...
    dom.append('g').attr('class', 'boxplot-median').attr('transform', `translate(${margins.left + scales.x.bandwidth()}, ${margins.top})`).selectAll('line').data(this.boxplotData).enter().append('line').attr('x1', d => scales.x(d.label) - scales.x.bandwidth() / 2).attr('y1', d => this.useLog ? scales.y(d.median + adjust) : scales.y(d.median)).attr('x2', d => scales.x(d.label) + scales.x.bandwidth() / 2).attr('y2', d => this.useLog ? scales.y(d.median + adjust) : scales.y(d.median)).attr('stroke', '#000').attr('stroke-width', 2);
    let whiskers = dom.append('g').attr('class', 'boxplot-whisker'); // render high whisker

    whiskers.append('g').attr('transform', `translate(${margins.left + scales.x.bandwidth()}, ${margins.top})`).selectAll('line').data(this.boxplotData).enter().append('line').attr('x1', d => scales.x(d.label)).attr('y1', d => this.useLog ? scales.y(d.q3 + adjust) : scales.y(d.q3)).attr('x2', d => scales.x(d.label)).attr('y2', d => this.useLog ? scales.y(d.upperBound + adjust) : scales.y(d.upperBound)).attr('stroke', '#aaa');
    whiskers.append('g').attr('transform', `translate(${margins.left + scales.x.bandwidth()}, ${margins.top})`).selectAll('line').data(this.boxplotData).enter().append('line').attr('x1', d => scales.x(d.label) - scales.x.bandwidth() / 4).attr('y1', d => this.useLog ? scales.y(d.upperBound + adjust) : scales.y(d.upperBound)).attr('x2', d => scales.x(d.label) + scales.x.bandwidth() / 4).attr('y2', d => this.useLog ? scales.y(d.upperBound + adjust) : scales.y(d.upperBound)).attr('stroke', '#aaa'); // render low whisker

    whiskers.append('g').attr('transform', `translate(${margins.left + scales.x.bandwidth()}, ${margins.top})`).selectAll('line').data(this.boxplotData).enter().append('line').attr('x1', d => scales.x(d.label)).attr('y1', d => this.useLog ? scales.y(d.q1 + adjust) : scales.y(d.q1)).attr('x2', d => scales.x(d.label)).attr('y2', d => this.useLog ? scales.y(d.lowerBound + adjust) : scales.y(d.lowerBound)).attr('stroke', '#aaa');
    whiskers.append('g').attr('transform', `translate(${margins.left + scales.x.bandwidth()}, ${margins.top})`).selectAll('line').data(this.boxplotData).enter().append('line').attr('x1', d => scales.x(d.label) - scales.x.bandwidth() / 4).attr('y1', d => this.useLog ? scales.y(d.lowerBound + adjust) : scales.y(d.lowerBound)).attr('x2', d => scales.x(d.label) + scales.x.bandwidth() / 4).attr('y2', d => this.useLog ? scales.y(d.lowerBound + adjust) : scales.y(d.lowerBound)).attr('stroke', '#aaa'); // render outliers

    dom.append('g').attr('class', 'boxplot-outliers').attr('transform', `translate(${margins.left + scales.x.bandwidth()}, ${margins.top})`).selectAll('g').data(this.boxplotData).enter().append('g').selectAll('circle').data(d => d.outliers.map(x => ({
      'label': d.label,
      'val': x
    }))).enter().append('circle').attr('cx', d => scales.x(d.label)).attr('cy', d => this.useLog ? scales.y(d.val + adjust) : scales.y(d.val)).attr('r', '2').attr('stroke', '#aaa').attr('fill', 'none');
  }
  ...
  _createSvg(rootId, width, height) {
    let svg = select(`#${rootId}`).append('svg').attr('width', width).attr('height', height);
    return svg;
  }

  _setScales(innerWidth, innerHeight, padding = 0) {
    let xScale = band().domain(this.boxplotData.map(d => d.label)).range([0, innerWidth]).paddingInner(padding);
    let yScale;

    if (this.useLog) {
      const adjust = this._getLogAdjustment();

      yScale = log().domain(extent(this.allVals).map(d => d + adjust)).range([innerHeight, 0]).base(this.logBase);
    } else {
      yScale = linear$1().domain(extent(this.allVals)).range([innerHeight, 0]);
    }
 ...
 * Licensed under the BSD 3-clause license (https://github.com/broadinstitute/gtex-viz/blob/master/LICENSE.md)
 */
function launch(rootId, gencodeId, urls = getGtexUrls()) {
  const promises = [json(urls.tissue), json(urls.geneExp + gencodeId)];
  Promise.all(promises).then(function (args) {
    const tissues = parseTissues(args[0]);
...
function createTissueMenu(domId, url = getGtexUrls().tissue) {
  json(url).then(function (results) {
...
function createTissueGroupMenu(groups, domId, forEqtl = false, checkAll = false, sections = 4) {
  ...
  const lastSiteClass = "last-site"; // erase everything in domId in case it isn't empty

  select(`#${domId}`).selectAll("*").remove(); // add check all and reset options

  const $allTissueDiv = jquery('<div/>').attr('class', 'col-xs-12 col-md-12').appendTo(jquery(`#${domId}`));

  if (forEqtl) {
    jquery(`<label class=${mainClass}>` + '<input type="radio" name="allTissues" value="reset"> Reset ' + '</label><br/>').appendTo($allTissueDiv);
  } else {
    jquery(`<label class=${mainClass}>` + '<input type="radio" name="allTissues" value="all"> All </label> ' + `<label class=${mainClass}>` + '<input type="radio" name="allTissues" value="reset"> Reset ' + '</label><br/>').appendTo($allTissueDiv);
  ...
  let groupNames = Object.keys(groups).sort((a, b) => {
    ...
    if (a == 'Brain') return -1;else if (b == 'Brain') return 1;else if (a < b) return -1;else if (a > b) return 1;
  ...
  groupNames.forEach(gname => {
    ...
    counter += groupLen;
    let $currentDom = $sections[currSection]; // create the <label> for the tissue group

    jquery(`<label class=${mainClass}>` + `<input type="checkbox" id="${gId}" class="tissueGroup"> ` + `<span>${gname}</span>` + '</label><br/>').appendTo($currentDom); // tissue sites in the group
    ...
    if (sites.length > 1) {
      ...
      sites.sort((a, b) => {
        ...
        let $siteDom = jquery(`<label class=${subClass}>` + `<input type="checkbox" id="${site.id}" class="tissueSubGroup"> ` + `<span>${site.name}</span>` + '</label><br/>').appendTo($currentDom);
...
function parseTissueGroupMenu(groups, domId, useNames = false) {
  let queryTissues = [];
  jquery(`#${domId}`).find(":input").each(function () {
...
class GroupedViolin {
  ...
  render(dom, width = 500, height = 357, xPadding = 0.05, xDomain = undefined, yDomain = [-3, 3], yLabel = "Y axis", showX = true, xAngle = 30, showSubX = true, subXAngle = 0, showWhisker = false, showDivider = false, showLegend = false, showSize = false, sortSubX = false, showOutliers = false, numPoints = 0) {
    ...
    this.groups = nest().key(d => d.group).entries(this.data);
    this.scale = {
      x: band().rangeRound([0, width]).domain(xDomain || this.groups.map(d => d.key)).paddingInner(xPadding),
      subx: band(),
      y: linear$1().rangeRound([height, 0]).domain(yDomain),
    ...
    this.groups.forEach(g => {
      ...
      if (info !== undefined) {
        ...
        .attr("x", 0).attr("y", 0).attr("class", "violin-group-label").attr("text-anchor", "center").attr("fill", d => {
      ...
      this.scale.subx.domain(entries.map(d => d.label)).rangeRound([this.scale.x(group), this.scale.x(group) + this.scale.x.bandwidth()]);
      ...
      if (showSize) {
        const sizeScale = band().domain(entries.map(d => {
          return d.size == undefined ? '(0)' : `(${d.size || 0})`;
        })).rangeRound([this.scale.x(group), this.scale.x(group) + this.scale.x.bandwidth()]);
        const sizexG = dom.append("g").attr("class", "violin-size-axis").attr("transform", `translate(0, ${height + buffer})`).call(axisBottom(sizeScale));
      }

      if (showSubX) {
        var buffer = 5;
        const subxG = dom.append("g").attr("class", "violin-sub-axis").attr("transform", `translate(0, ${height + buffer})`).call(axisBottom(this.scale.subx));

        if (subXAngle > 0) {
          subxG.selectAll("text").attr("text-anchor", "start").attr("transform", `rotate(${subXAngle}, 2, 10)`);
        }
      }
    }); // renders the x axis
    ...
    dom.append("g").attr("class", "violin-x-axis axis--x").attr("transform", `translate(0, ${height + buffer})`).call(this.xAxis) // set tickFormat("") to show tick marks without text labels
    .selectAll("text").attr("text-anchor", "start").attr("transform", `rotate(${xAngle}, -10, 10)`); // adds the y Axis

    buffer = 5;
    this.yAxis = axisLeft(this.scale.y).tickValues(this.scale.y.ticks(5));
    dom.append("g").attr("class", "violin-y-axis axis--y").attr("transform", `translate(-${buffer}, 0)`).call(this.yAxis); // adds the text label for the y axis

    dom.append("text").attr("class", "violin-axis-label").attr("text-anchor", "middle").attr("transform", `translate(-${buffer * 2 + select('.violin-y-axis').node().getBBox().width}, ${this.scale.y.range()[0] + (this.scale.y.range()[1] - this.scale.y.range()[0]) / 2}) rotate(-90)`).text(yLabel); // plot mouse events
    ...
    if (showLegend) {
      const legendG = dom.append("g").attr("id", "violinLegend").attr("transform", `translate(0, 0)`);
  ...
  zoom(dom, theBrush) {
    ...
    this.groups.forEach((gg, i) => {
      let group = gg.key;
      let entries = gg.values; // re-define the subx's range

      this.scale.subx.rangeRound([this.scale.x(group), this.scale.x(group) + this.scale.x.bandwidth()]);
      entries.forEach(entry => {
        if (0 == entry.values.length) return; // no further rendering if this group has no entries

        const gIndex = this.scale.x.domain().indexOf(group); // re-define the scale.z's range

        this.scale.z.range([this.scale.subx(entry.label), this.scale.subx(entry.label) + this.scale.subx.bandwidth()]); // re-render the violin

        const g = dom.select(`#violin${gg.index}-${entry.label}`);
        g.select(".violin").transition(t).attr("d", area().x0(d => this.scale.z(d[1])).x1(d => this.scale.z(-d[1])).y(d => this.scale.y(d[0]))); // re-render the box plot
        // interquartile range

        const q1 = quantile(entry.values, 0.25);
        const q3 = quantile(entry.values, 0.75);
        const z = 0.1;
        g.select(".violin-ir").transition(t).attr("x", this.scale.z(-z)).attr("y", this.scale.y(q3)).attr("width", Math.abs(this.scale.z(-z) - this.scale.z(z))).attr("height", Math.abs(this.scale.y(q3) - this.scale.y(q1))); // the median line
  ...
  _drawViolin(dom, entry, showWhisker, gIndex, showOutliers, numPoints) {
    ...
    let kde = kernelDensityEstimator(kernel.gaussian, this.scale.y.ticks(100), // use up to 100 vertices along the Y axis (to create the violin path)
    ...
    const violinG = dom.append("g").attr('id', `violin${gIndex}-${entry.label}`).attr('class', 'violin-g').datum(entry); // violin plot and box can only be drawn when vertices exist and there are no NaN points
    ...
    if (entry.values.length >= numPoints && vertices.length && this._validVertices(vertices)) {
      ...
      this.scale.z.domain([-zMax, zMax]).range([this.scale.subx(entry.label), this.scale.subx(entry.label) + this.scale.subx.bandwidth()]); // visual rendering

      let violin = area().x0(d => this.scale.z(d[1])).x1(d => this.scale.z(-d[1])).y(d => this.scale.y(d[0]));
      const vPath = violinG.append("path").datum(vertices).attr("d", violin).classed("violin", true).style("fill", () => {
        if (entry.color !== undefined) return entry.color; // alternate the odd and even colors, maybe we don't want this feature
      ...
      if (showWhisker) {
        ...
        dom.append("line").classed("whisker", true).attr("x1", this.scale.z(0)).attr("x2", this.scale.z(0)).attr("y1", this.scale.y(upper)).attr("y2", this.scale.y(lower)).style("stroke", "#fff");
      } // interquartile range


      violinG.append("rect").attr("x", this.scale.z(-z)).attr("y", this.scale.y(q3)).attr("width", Math.abs(this.scale.z(-z) - this.scale.z(z))).attr("height", Math.abs(this.scale.y(q3) - this.scale.y(q1))).attr("class", "violin-ir"); // median

      const med = median(entry.values);
      violinG.append("line") // the median line
      .attr("x1", this.scale.z(-z)).attr("x2", this.scale.z(z)).attr("y1", this.scale.y(med)).attr("y2", this.scale.y(med)).attr("class", "violin-median"); // outliers
      ...
      if (showOutliers) {
        ...
        violinG.append("g").attr("class", "violin-outliers").selectAll("circle").data(outliers).enter().append("circle") // .attr("cx", ()=>this.scale.z(0))
        .attr("cx", () => this.scale.z(jitter())).attr("cy", d => this.scale.y(d)).attr("r", 2);
      } // mouse events


      violinG.on("mouseover", () => {
        vPath.classed("highlighted", true); // console.log(entry);
      ...
      const z = this.scale.z.domain()[1] / 3;
      let jitter = randomNormal(0, z / 2);
      violinG.append("g").attr("class", "violin-points").selectAll("circle").data(entry.values).enter().append("circle").attr("cx", () => this.scale.z(jitter())).attr("cy", d => this.scale.y(d)).attr("r", 2);
    }
  }
  ...
  _addGroupDivider(dom) {
    ...
    dom.selectAll(".vline").data(groups).enter().append("line").classed("vline", true).attr("x1", getX).attr("x2", getX).attr("y1", this.scale.y.range()[0]).attr("y2", this.scale.y.range()[1]).style("stroke-width", (g, i) => i != groups.length - 1 ? 1 : 0).style("stroke", "rgb(86,98,107)").style("opacity", 0.5);
  }
 ...
 * Licensed under the BSD 3-clause license (https://github.com/broadinstitute/gtex-viz/blob/master/LICENSE.md)
 */
function launch$1(rootId, tooltipRootId, gencodeId, plotTitle = "Gene Expression Violin Plot", urls = getGtexUrls(), margins = _setViolinPlotMargins(50, 75, 250, 60), dimensions = {
  w: window.innerWidth * 0.8,
  h: 250
}) {
  const promises = [json(urls.tissue), json(urls.geneExp + gencodeId), json(urls.geneExp + gencodeId + '&attributeSubset=sex')];
  const ids = {
    root: rootId,
    spinner: 'spinner',
    svg: `${rootId}-svg`,
  ...
  if ($(`#${ids.root}`).length == 0) throw 'Violin Plot Error: rootId does not exist.'; // create DOM components if not already present
  ...
  Promise.all(promises).then(function (args) {
    ...
    let tooltip = violinPlot.createTooltip(ids.tooltip); // adding properties to keep track of sorting and filtering specifically for this plot
    ...
    violinPlot.unit = violinPlotData.length > 0 ? ` ${violinPlotData[0].unit}` : ''; // default plot options
...
function _addToolbar(vplot, tooltip, ids, urls) {
  ...
  toolbar.createDownloadSvgButton(ids.buttons.download, ids.svg, 'gene-exp-plot.svg', ids.clone); // adding bootstrap classes to toolbar

  $(`#${ids.toolbar}`).addClass('row');
  $(`#${ids.toolbar} .btn-group`).addClass('col-xs-12 col-lg-1 text-nowrap').css('display', 'flex');
  $('<div></div>').appendTo(`#${ids.toolbar}`).attr('id', `${ids.toolbar}-plot-options`).attr('class', 'col-lg-11 text-nowrap');
  let plotOptions = $(`#${ids.toolbar}-plot-options`); // subsetting options

  $('<div/>').appendTo(plotOptions).attr('id', ids.plotOptionGroups.differentiation).attr('class', 'col-lg-2 col-xl-2');
  $('<span/>').appendTo(`#${ids.plotOptionGroups.differentiation}`).attr('class', `${ids.root}-option-label`).html('Subset');
  $('<div/>').appendTo(`#${ids.plotOptionGroups.differentiation}`).attr('class', 'btn-group btn-group-sm');
  ...
  $(`<button class="btn btn-default" id="${ids.buttons.sexDiff}">Sex</button>`).appendTo(subsetButtonGroup); // scale options

  $('<div/>').appendTo(plotOptions).attr('id', ids.plotOptionGroups.scale).attr('class', 'col-lg-2 col-xl-2');
  $('<span/>').appendTo(`#${ids.plotOptionGroups.scale}`).attr('class', `${ids.root}-option-label`).html('Scale');
  $('<div/>').appendTo(`#${ids.plotOptionGroups.scale}`).attr('class', 'btn-group btn-group-sm');
  ...
  $(`<button class="btn btn-default" id="${ids.buttons.linearScale}">Linear</button>`).appendTo(scaleButtonGroup); // sort options -- tissue name sorts

  $('<div/>').appendTo(plotOptions).attr('class', `${ids.plotOptionGroups.sort} col-lg-2 col-xl-2`).attr('id', `vplot-alpha-sorts`);
  $('<span/>').appendTo(`.${ids.plotOptionGroups.sort}#vplot-alpha-sorts`).attr('class', `${ids.root}-option-label`).html('Tissue Sort');
  $('<div/>').appendTo(`.${ids.plotOptionGroups.sort}#vplot-alpha-sorts`).attr('class', 'btn-group btn-group-sm').attr('id', `${ids.plotOptionGroups.sort}-alpha`);
  ...
  $(`<button class="btn btn-default fa fa-sort-alpha-up" id="${ids.buttons.descAlphaSort}"></button>`).appendTo(alphaSortButtonGroup); // sort options -- median sorts

  $('<div/>').appendTo(plotOptions).attr('class', `${ids.plotOptionGroups.sort} col-lg-2 col-xl-2`).attr('id', `vplot-num-sorts`);
  $('<span/>').appendTo(`.${ids.plotOptionGroups.sort}#vplot-num-sorts`).attr('class', `${ids.root}-option-label`).html('Median Sort');
  $('<div/>').appendTo(`.${ids.plotOptionGroups.sort}#vplot-num-sorts`).attr('class', 'btn-group btn-group-sm').attr('id', `${ids.plotOptionGroups.sort}-num`);
  ...
  $(`<button class="btn btn-default fa fa-sort-numeric-up" id="${ids.buttons.descSort}"></button>`).appendTo(numSortButtonGroup); // outlier display options

  $('<div/>').appendTo(plotOptions).attr('id', ids.plotOptionGroups.outliers).attr('class', 'col-lg-2 col-xl-2');
  $('<span/>').appendTo(`#${ids.plotOptionGroups.outliers}`).attr('class', `${ids.root}-option-label`).html('Outliers');
  $('<div/>').appendTo(`#${ids.plotOptionGroups.outliers}`).attr('class', 'btn-group btn-group-sm');
  ...
  selectAll(`#${ids.plotOptionsModal} .modal-body button`).classed('active', false); // plot defaults

  selectAll(`#${ids.buttons.ascAlphaSort},
               #${ids.buttons.linearScale},
               #${ids.buttons.noDiff},
               #${ids.buttons.outliersOn}`).classed('active', true); // filter

  toolbar.createButton(ids.buttons.filter, 'fa-filter');
  let tissueFilterButton = select(`#${ids.buttons.filter}`).on('mouseover', () => {
...
function _calcViolinPlotValues(data, useLog = true) {
  data.forEach(d => {
    ...
    d.values.sort(ascending$1); // median needed for sorting
...
function _addViolinTissueColorBand(plot, dom, tissueDict, loc = "top") {
  _moveXAxis(dom); // moving x-axis text down to make space for color band


  const xAxisText = dom.selectAll('.violin-x-axis text');
  xAxisText.attr('transform', `translate(0, 8) ${xAxisText.attr('transform')}`); // add tissue colors

  const tissueG = dom.append("g");
  tissueG.selectAll(".tcolor").data(plot.scale.x.domain()).enter().append("rect").classed("tcolor", true).attr("x", g => plot.scale.x(g)).attr("y", g => loc == "top" ? plot.scale.y.range()[1] : plot.scale.y.range()[0]).attr('transform', 'translate(0, 14)').attr("width", g => plot.scale.x.bandwidth()).attr("height", 5).style("stroke-width", 0).style("fill", g => `#${tissueDict[g].colorHex}`).style("opacity", 0.9);
}
/**
...
function _customizeTooltip(vplot) {
  ...
  violinGs.on('mouseover', (d, i, nodes) => {
    ...
    if (!vplot.gpConfig.subset) {
      vplot.tooltip.show(d.group + "<br/>" + `n = ${d.values.length}` + "<br/>" + `Median${vplot.unit}: ${med.toPrecision(4)}` + "<br/>");
    } else {
      vplot.tooltip.show(d.group + "<br/>" + d.label + ` (n = ${d.values.length})` + "<br/>" + `Median${vplot.unit}: ${med.toPrecision(4)}` + "<br/>");
    }
  });
}
/**
...
function _redrawViolinPlot(vplot, ids) {
  ...
  let xDomain = _sortData(vplot.gpConfig.sort, sortData, ids); // scaling
...
function launchTopExpressed(tissueId, heatmapRootId, violinRootId, urls = getGtexUrls(), filterGenes = true) {
  ...
  json(url + tissueId).then(function (results) {
...
function launch$2(formId, menuId, submitId, heatmapRootId, violinRootId, urls = getGtexUrls(), callback = undefined) {

  json(urls.tissueSites).then(function (data) {
    ...
    $(`#${submitId}`).click(function () {
      ...
      if (callback !== undefined) callback(); // hide the search form after the eQTL violin plots are reported
      ...
      let queryTissueIds = parseTissueGroupMenu(tissueGroups, menuId); // tissue input error-checking
...
function searchById(heatmapRootId, violinRootId, glist, tlist = undefined, urls = getGtexUrls(), filterGenes = undefined, callback = undefined, qTissue = undefined) {
  ...
  Promise.all([json(urls.tissue), json(urls.geneId + glist.join(","))]).then(function (args) {
    const tissues = parseTissues(args[0]); // genes

    const genes = parseGenes(args[1]); // error-checking

    message += _validateGenes(heatmapRootId, genes, glist); // get median expression data and clusters of the input genes in all tissues
    ...
    json(fetchUrl).then(function (eData) {
      $('#spinner').hide();
      ...
      if (dataMessage !== undefined) {
        ...
        select("#" + dmap.config.panels.main.id).selectAll(".exp-map-ylabel").text(d => geneDict[d] === undefined ? d : geneDict[d].geneSymbol); // Add tissue color boxes //

        _addTissueColors(dmap, tissueDict); // Add a toolbar


        _addToolBar(dmap, ids, tissueDict, urls, filterGenes, qTissue); // mouse events
...
function _addTissueColors(dmap, tissueDict) {
  ...
  let leaves = select(`#${id}`).selectAll(".leaf-color").data(heatmap.xList); // update

  cells.attr("x", d => heatmap.xScale(d)).attr("y", d => heatmap.yScale.range()[1] + 5);
  leaves.attr("x", d => heatmap.xScale(d)).attr("y", d => heatmap.yScale.range()[0] - 10); // create new elements

  cells.enter().append("rect").attr("x", d => heatmap.xScale(d)).attr("y", d => heatmap.yScale.range()[1] + 5).attr("width", heatmap.xScale.bandwidth()).attr("height", heatmap.yScale.bandwidth() * 0.5).classed("exp-map-xcolor", true).merge(cells).style("fill", d => tissueDict[d] === undefined ? "#000000" : `#${tissueDict[d].colorHex}`); // exit and remove

  cells.exit().remove();

  if (dmap.objects.heatmap.yScale.domain().length > 15) {
    leaves.enter().append("rect").attr("x", d => heatmap.xScale(d)).attr("y", d => heatmap.yScale.range()[0] - 10).attr("width", heatmap.xScale.bandwidth()).attr("height", heatmap.yScale.bandwidth() * 0.5).classed("leaf-color", true).merge(leaves).style("fill", d => tissueDict[d] === undefined ? "#000000" : `#${tissueDict[d].colorHex}`);
    leaves.exit().remove();
  }
}
/**
...
function _customizeMouseEvents(dmap, tissueDict, geneDict, urls = getGtexUrls()) {
  ...
  const ylabelClick = function (d) {
    let s = select(this);
    let action = ""; // toggles click/unclick events
...
function _renderViolinHelper(data, dmap, tissueDict) {
  ...
  select(rootId).selectAll("*").remove(); // rebuild the dom components within the root div
  ...
  if (data.length == 0) {
    // no expression data, no need to proceed
  ...
  width = width < dmap.config.panels.main.w ? dmap.config.panels.main.w : width; // width = width < 300?300: width;

  const dim = _setViolinPlotDimensions$1(width, 150, margin); // render the violin


  const dom = select(`#${id.chart}`) // .style("opacity", 1.0)
  .append("svg").attr("width", dim.outerWidth).attr("height", dim.outerHeight).attr("id", id.svg).append("g").attr("transform", `translate(${margin.left}, ${margin.top})`);
  ...
  violin.render(dom, dim.width, dim.height, 0.30, tissueOrder.map(d => d.id), [], "log10(TPM)", true, 30, false, 0, false, showDivider, true); // check and adjust the svg width
...
function _setViolinPlotDimensions$1(width = 1200, height = 250, margin = _setMargins()) {
  return {
    width: width,
    height: height,
    outerWidth: width + (margin.left + margin.right),
...
function _addViolinTissueColorBand$1(plot, dom, tissueDict, loc = "top") {
  ///// add tissue colors
  const tissueG = dom.append("g");
  tissueG.selectAll(".tcolor").data(plot.scale.x.domain()).enter().append("rect").classed("tcolor", true).attr("x", g => plot.scale.x(g)).attr("y", g => loc == "top" ? plot.scale.y.range()[1] - 5 : plot.scale.y.range()[0] - 5).attr("width", g => plot.scale.x.bandwidth()).attr("height", 5).style("stroke-width", 0).style("fill", g => `#${tissueDict[g].colorHex}`).style("opacity", 0.7);
}
...
function _addToolBar(dmap, ids, tissueDict, urls = getGtexUrls(), filterGenes = undefined, qTissue = undefined) {
  ...
  const __addSortTissues = () => {
    ...
    select(`#${id}`).on("click", function () {
      ...
      select("#" + dmap.config.panels.top.id).style("display", "None"); // sort tissues
...
function _sortTissues(xlist, dmap, tissueDict) {
  ...
  if (!qTissue.empty()) qId = qTissue.datum(); // update the heatmap

  const dom = select("#" + dmap.config.panels.main.id);
  const dimensions = dmap.config.panels.main;
  dmap.objects.heatmap.redraw(dom, xlist, dmap.objects.heatmap.yList, dimensions); // change the tissue display text to tissue names

  selectAll(".exp-map-xlabel").text(d => tissueDict[d] === undefined ? d : tissueDict[d].tissueSiteDetail).classed("query", false);

  _addTissueColors(dmap, tissueDict); // style the query tissue if found


  if (qId !== undefined) _styleSelectedTissue(qId); // hide the violin plot

  select('#violinRoot').style("opacity", 0.0); // deselect genes
...
class BubbleMap {
  constructor(data, useLog = true, logBase = 10, colorScheme = "Reds") {
    ...
    this.useLog = useLog;
    this.logBase = logBase;
    this.colorScheme = colorScheme; // initiates additional attributes
  ...
  drawCanvas(canvas, dimensions = {
    w: 1000,
    h: 600,
    top: 20,
    left: 20
  }, colorScaleDomain = undefined, labelConfig = {
    column: {
      show: true,
      angle: 30,
      adjust: 0,
    ...
    let context = canvas.node().getContext('2d'); //background

    context.fillStyle = '#ffffff';
    context.rect(0, 0, canvas.attr('width'), canvas.attr('height'));
    context.fill(); // bubbles
  ...
  drawCombo(miniDom, focusDom, dimensions = {
    w: 1000,
    h: 600,
    top: 0,
    left: 0
  }, colorScaleDomain = undefined, addBrush = true, labelConfig = {
    column: {
      show: true,
      angle: 30,
      adjust: 0,
    ...
    let bubbles = miniDom.append("g").attr("clip-path", "url(#clip)");

    this._setMiniScales(dimensions, colorScaleDomain);

    bubbles.selectAll(".mini-map-cell").data(this.data, d => d.value).enter().append("circle").attr('class', 'mini-map-cell').attr("row", d => `x${this.xScaleMini.domain().indexOf(d.x)}`).attr("col", d => `y${this.yScaleMini.domain().indexOf(d.y)}`).attr("cx", d => this.xScaleMini(d.x) + this.xScaleMini.bandwidth() / 2).attr("cy", d => this.yScaleMini(d.y)).attr("r", d => this.bubbleScaleMini(d.r)).style("fill", d => this.colorScale(d.value));

    if (addBrush) {
      let brush = brushX().extent([[0, 0], [dimensions.w, dimensions.h]]).on("brush", () => {
        this._brushed(focusDom, labelConfig);
      });
      miniDom.append("g").attr("class", "brush").call(brush) // .call(brush.move, this.xScaleMini.range());
  ...
  drawSvg(dom, dimensions = {
    ...
    top: 0,
    left: 0
  }, colorScaleDomain = undefined, brushSize = 50, labelConfig = {
    column: {
      show: true,
      angle: 30,
      adjust: 0,
    ...
    let tooltip = this.tooltip; // bubbles

    let bubbles = dom.append("g").attr("clip-path", "url(#clip)");
    bubbles.selectAll(".bubble-map-cell").data(this.data, d => d.value).enter().append("circle").attr("class", "bubble-map-cell").attr("row", d => `x${this.xScale.domain().indexOf(d.x)}`).attr("col", d => `y${this.yScale.domain().indexOf(d.y)}`).attr("cx", d => this.xScale(d.x) + this.xScale.bandwidth() / 2).attr("cy", d => this.yScale(d.y) + this.yScale.bandwidth() / 2).attr("r", d => this.bubbleScale(d.r)).style("fill", d => this.colorScale(d.value)).on("mouseover", function (d) {
      let selected = select(this);
      let rowClass = selected.attr("row");
      let colClass = selected.attr("col");
      dom.selectAll(".bubble-map-xlabel").filter(`.${rowClass}`).classed('highlighted', true);
      dom.selectAll(".bubble-map-ylabel").filter(`.${colClass}`).classed('highlighted', true);
      selected.classed('highlighted', true);
    ...
    if (cl.show) {
      // column labels
      let lookup = {};
      nest().key(d => d.x) // group this.data by d.x
      .entries(this.data).forEach(d => {
        lookup[d.key] = d.values[0].displayX;
      });
      let xLabels = dom.selectAll('.bubble-map-xlabel').data(this.xScale.domain()).enter().append("text").attr("class", (d, i) => `bubble-map-xlabel x${i}`).attr("x", 0).attr("y", 0).attr("text-anchor", cl.textAlign == 'left' ? 'start' : 'end').style("cursor", "default").style("font-size", () => {
    ...
    if (rl.show) {
      // row labels
      let lookup = {};
      nest().key(d => d.y) // group this.data by d.y
      .entries(this.data).forEach(d => {
        lookup[d.key] = d.values[0].displayY;
      });
      let yLabels = dom.selectAll('.bubble-map-ylabel').data(this.yScale.domain()).enter().append("text").attr("class", (d, i) => `bubble-map-ylabel y${i}`).attr("x", 0).attr("y", 0).attr("text-anchor", rl.textAlign == 'left' ? 'start' : 'end').style("cursor", "default").style("font-size", () => {
  ...
  drawBubbleLegend(dom, legendConfig = {
    x: 0,
    y: 0,
    title: "Bubble legend"
  }, ticks = 5, unit = "") {
    ...
    let legendG = dom.append("g").attr("class", "bmap-bubble-legend").attr("transform", `translate(${legendConfig.x}, ${legendConfig.y})`); // legend title

    legendG.append("text").attr("class", "color-legend").text(legendConfig.title).attr("x", -10).attr("text-anchor", "end").attr("y", 10);
    let legends = legendG.selectAll(".legend").data(data);
    let g = legends.enter().append("g").classed("legend", true); // the bubbles

    let cellW = 40;
    g.append("circle").attr("cx", (d, i) => cellW * i).attr("cy", 10).attr("r", d => this.bubbleScale(d)).style("fill", "black");
    g.append("text").attr("class", "color-legend").text(d => this.useLog ? Math.pow(base, d).toPrecision(2) : d.toPrecision(2)).attr("x", (d, i) => cellW * i - 5).attr("y", 0);
  } // private methods
  ...
  _brushed(focusDom, labelConfig) {
    ...
    this.bubbleScale = this._setBubbleScale({
      max: bubbleMax,
      min: 2
    }); // TODO: change hard-coded min radius
    // update the focus bubbles

    focusDom.selectAll(".bubble-map-cell").attr("cx", d => {
    ...
    focusDom.selectAll(".bubble-map-xlabel").attr("transform", d => {
  ...
  _setMiniScales(dimensions = {
    ...
    if (this.bubbleScaleMini === undefined) {
      ...
      this.bubbleScaleMini = this._setBubbleScale({
        max: bubbleMax,
        min: 1
      });
    }
  ...
  _parseXList() {
    let xList = nest().key(d => d.x) // group this.data by d.x
  ...
  _parseYList() {
    let yList = nest().key(d => d.y) // group this.data by d.x
    .entries(this.data).map(d => d.key) // then return the unique list of d.x
    .sort((a, b) => {
      return a < b ? -1 : a > b ? 1 : a >= b ? 0 : NaN;
    });
  ...
  _setXScale(dim = {
    ...
    .range([dim.left, dim.left + dim.w]).padding(.05); // temporarily hard-coded value
  }
  ...
  _setYScale(dim = {
    ...
    .range([dim.top, dim.top + dim.h]).padding(.05); // temporarily hard-coded value
  }
  ...
  _setBubbleScale(range = {
    max: 10,
    min: 0
  }) {
    return sqrt().domain([3, max(this.data.map(d => d.r))]) // set min at 2 for -log(0.01)
...
class HalfMap {
  ...
  constructor(data, cutoff = 0.0, useLog = true, logBase = 10, colorScheme = "Greys", colorScaleDomain = [0, 1]) {
    ...
    this.useLog = useLog;
    this.logBase = logBase;
    this.colorScheme = colorScheme; // color scale normally doesn't change with the same data set
    // therefore can be defined at instantiation

    this.colorScale = this._setColorScale(colorScaleDomain); // the following scales could change depending on the user defined dimensions
    // therefore they are undefined at instantiation

    this.xScale = undefined;
    this.yScale = undefined;
    this.labelScale = undefined; // peripheral features
  ...
  draw(canvas, svg, dimensions = {
    ...
    top: 20,
    left: 20
  }, colorScaleDomain = [0, 1], showLabels = true, labelAngle = 90, xScaleDomain = undefined, yScaleDomain = undefined) {
  ...
  _drawCanvas(canvas, dimensions = {
    ...
    let context = canvas.node().getContext('2d'); // transform the canvas
    ...
    context.clearRect(-dimensions.w, -dimensions.w, dimensions.w * 2, dimensions.w * 2); // LD canvas rendering from GEV old code
    ...
    this.filteredData.forEach(d => {
      ...
      context.fillStyle = this.colorScale(d.value);
      context.fillRect(x, y, this.xScale.bandwidth(), this.yScale.bandwidth()); // uncomment the following for debugging
  ...
  drawSvg(svg, dimensions, drawCells = true, showLabels = true, labelAngle = 90, colorScaleDomain = [0, 1], xScaleDomain, yScaleDomain = undefined) {
    if (drawCells) {
      ...
      let cells = mapG.selectAll(".half-map-cell").data(this.filteredData); // add new rects
    ...
    select(svg.node().parentNode).style("cursor", "none").style("position", "absolute").on('mousemove', () => {
      ...
      let x = pos[0];
      let y = pos[1]; // find the colliding cell's coordinates (before transformation)
      ...
      let j = Math.floor(y2 / this.yScale.step()); // show tooltip

      let col = this.xScale.domain()[i];
      let row = this.yScale.domain()[j];
      let cell = this.dataDict[col + row]; // console.log([x, y, x2, y2, col, row]); // debugging
...
function groupedViolinPlot(par = violinDemoConfig) {
  ...
  let tooltipId = `${par.id}Tooltip`; // create the SVG

  let svg = createSvg(par.id, par.width, par.height, margin);
  const gViolin = new GroupedViolin(par.data);
  gViolin.render(svg, inWidth, inHeight, par.xPadding, undefined, [], par.yLabel, par.showX, par.xAngle, par.showSubX, par.subXAngle, par.showWhisker, par.showDivider, par.showLegend, par.showSampleSize, par.sortSubX, par.showOutliers, par.numPoints);
 ...
 * Licensed under the BSD 3-clause license (https://github.com/broadinstitute/gtex-viz/blob/master/LICENSE.md)
 */
function render$1(par, gencodeId, variantId, tissueId, groupName = undefined, urls = getGtexUrls()) {
  json(urls['dyneqtl'] + `?variantId=${variantId}&gencodeId=${gencodeId}&tissueSiteDetailId=${tissueId}`).then(function (json) {
    let data = parseDynEqtl(json); // construct the dynEqtl data for the three genotypes: ref, het, alt
 ...
 * Licensed under the BSD 3-clause license (https://github.com/broadinstitute/gtex-viz/blob/master/LICENSE.md)
 */
function render$2(par, geneId, urls = getGtexUrls()) {
  $(`#${par.divSpinner}`).show();
  json(urls.geneId + geneId) // query the gene by geneId which could be gene name or gencode ID with or withour versioning
  .then(data => {
    let gene = parseGenes(data, true, geneId); // report the gene info

    $(`#${par.divGeneInfo}`).empty();
    $("<span/>").html(`<span>${gene.geneSymbol} (${gene.gencodeId}), Chr${gene.chromosome}:${gene.start} - ${gene.end} (${gene.strand}), ${gene.description}`).appendTo($(`#${par.divGeneInfo}`));
    let promises = [json(urls.tissueSummary), json(urls.tissueSites), json(urls.exon + gene.gencodeId), json(urls.singleTissueEqtl + gene.gencodeId)];
    Promise.all(promises).then(function (results) {
      ...
      let bmap = renderBubbleMap(par, gene, tissues, exons, tissueSiteTable, urls); // fetch LD data, this query is slow, so it's not included in the promises.

      json(urls.ld + gene.gencodeId).then(ldJson => {
        let ld = parseLD(ldJson);
        par.ldData = ld.filter(d => d.value >= par.ldCutoff); // filter unused data

        renderLdMap(par, bmap);
        $(`#${par.divSpinner}`).hide(); // define the tissue filtering event on tissue menu window close
        // Note: the tissue menu content in the modal is built by renderBmapFilters()

        const oriY = bmap.yScale.domain();
        const oriX = bmap.xScale.domain(); // execute the tissue filtering when the modal window closes
        ...
        $(`#${par.divModal}`).on('hidden.bs.modal', e => {
          ...
          let newX = nest().key(d => d.x) // group this.data by d.x
          .entries(par.data).map(d => d.key) // then return the unique list of d.x
          .sort((a, b) => {
            return a < b ? -1 : a > b ? 1 : a >= b ? 0 : NaN;
          }); // re-rendering
...
function setDimensions(par) {
  ...
  let hMax = 18;
  let hMin = 10;
  if (h < hMin) par.height = hMin * yList.length + par.margin.top + par.margin.bottom + par.miniPanelHeight + par.legendHeight;else if (h > hMax) par.height = hMax * yList.length + par.margin.top + par.margin.bottom + par.miniPanelHeight + par.legendHeight;
...
function createSvg$1(rootId, width, height, svgId = undefined) {
  checkDomId(rootId);
  if (svgId === undefined) svgId = `${rootId}-svg`;
  select(`#${svgId}`).remove(); // remove previously rendered svg if found

  let svg = select("#" + rootId).append("svg").attr("width", width).attr("height", height).attr("id", svgId);
...
function renderLdMap(par, bmap) {
  ...
  let ldCanvas = select(`#${par.ldId}`).append("canvas").attr("id", par.ldId + "-ld-canvas").attr("width", par.width).attr("height", par.width).style("position", "absolute");
  let ldSvg = createSvg$1(par.ldId, par.width, par.width, undefined);
  let ldG = ldSvg.append("g").attr("class", "ld").attr("id", "ldG").attr("transform", `translate(${par.ldPanelMargin.left}, ${par.ldPanelMargin.top})`);
  ldMap.drawColorLegend(ldSvg, {
    x: par.ldPanelMargin.left,
    y: 100
  }, 10, "LD");
  ...
  let ldConfig = {
    w: par.inWidth,
    top: par.ldPanelMargin.top,
    left: par.ldPanelMargin.left
  };
  ldMap.draw(ldCanvas, ldG, ldConfig, [0, 1], false, undefined, bmap.xScale.domain(), bmap.xScale.domain()); // update the brush event on the mini bubble map after LD map is rendered
...
function renderBubbleMap(par, gene, tissues, exons, tissueSiteTable, urls, update = false) {
  ...
  bmap.addTooltip(par.id); ////// Custom attributes added to bmap //////
  ...
  bmap.rsLookUp = {};
  bmap.varLookUp = {};
  let bmapSvg = createSvg$1(par.id, par.width, par.height, undefined);
  let miniG = bmapSvg.append("g") // global bubble map <g>
  .attr("class", "context").attr("id", "miniG").attr("transform", `translate(${par.margin.left}, ${par.margin.top})`);
  let focusG = bmapSvg.append("g") // zoomed bubble map <g>
  .attr("id", "focusG").attr("class", "focus").attr("transform", `translate(${par.focusPanelMargin.left}, ${par.focusPanelMargin.top})`);
  bmap.drawCombo(miniG, focusG, {
    w: par.inWidth,
    h: par.miniPanelHeight,
    top: 5,
    left: 0,
    h2: par.focusPanelHeight
  }, par.colorScaleDomain, false, // do not use the default brush, use a custom brush defined below
  par.focusPanelLabels);
  $(`#${par.divInfo}`).text('Total eQTL counts: ' + par.data.length); ///// Below are custom features and functionality
  //-- override bubble mouseover tooltip

  focusG.selectAll(".bubble-map-cell").on("mouseover", function (d) {
    let selected = select(this);
    let rowClass = selected.attr("row");
    let colClass = selected.attr("col");
    focusG.selectAll(".bubble-map-xlabel").filter(`.${rowClass}`).classed('highlighted', true);
    focusG.selectAll(".bubble-map-ylabel").filter(`.${colClass}`).classed('highlighted', true);
    selected.classed('highlighted', true);
    let displayValue = d.displayValue === undefined ? parseFloat(d.value.toExponential()).toPrecision(4) : d.displayValue;
    let displaySize = d.rDisplayValue === undefined ? d.r.toPrecision(4) : d.rDisplayValue;
    let displayX = d.displayX === undefined ? d.x : d.displayX;
  ...
  renderBmapFilters(par.divDashboard, par.divInfo, par.divModal, bmap, bmapSvg, tissueSiteTable); // variant related data parsing
  // Variant locator

  buildVariantLookupTables(bmap); //-- identify variants that are in the exon regions

  bmap.variantsInExons = findVariantsInExonRegions(bmap.xScale.domain(), exons); //-- tissue badges, which report the tissue sample counts next to the tissue row labels

  renderTissueBadges(tissues, bmap, bmapSvg); //-- TSS and TES markers
  ...
  renderTssDistanceTrack(gene, bmap, bmapSvg); //-- Add the click event for the bubbles: pop a dialog window and render the eQTL violin plot

  addBubbleClickEvent(bmap, bmapSvg, par); //-- add the focus view brush and defint the brush event

  bmap.brushEvent = () => {
    // update all the variant related visual features
    // -- focus view of the heat map
    let focusDomain = updateFocusView(par, bmap, bmapSvg); // -- gene TSS and TES markers

    if (bmap.tss && bmap.xScale(bmap.tss) || bmap.tes && bmap.xScale(bmap.tes)) renderGeneStartEndMarkers(bmap, bmapSvg, false); // -- TSS distance track

    renderTssDistanceTrack(gene, bmap, bmapSvg);
    return focusDomain;
  };

  bmap.brush = brushX().extent([[0, 0], [par.inWidth, par.miniPanelHeight + 5]]).on("brush", bmap.brushEvent);
  bmap.drawColorLegend(bmapSvg, {
    x: par.focusPanelMargin.left,
    y: par.focusPanelMargin.top - 50
  }, 4, "NES");
  miniG.append("g").attr("class", "brush").call(bmap.brush).call(bmap.brush.move, [0, bmap.xScaleMini.bandwidth() * 80]); // set the brush size to 60 columns
...
function updateFocusView(par, bmap, bmapSvg) {
  ...
  let brushRight = Math.round(selection[1] / bmap.xScaleMini.step()); // update scales
  ...
  bmap.drawBubbleLegend(bmapSvg, {
    x: par.width / 2,
    y: par.focusPanelMargin.top - 50,
    title: "-log10(p-value)"
  }, 5, "-log10(p-value)"); // update the focus bubbles

  bmapSvg.select("#focusG").selectAll(".bubble-map-cell").attr("cx", d => {
  ...
  bmapSvg.select("#focusG").selectAll(".bubble-map-xlabel").attr("transform", d => {
...
function renderTissueBadges(tissues, bmap, bmapSvg) {
  let badges = bmapSvg.select('#focusG').append('g').attr('id', 'tissueBadgeG').selectAll('.tissue-badge').data(tissues.filter(d => {
    return bmap.yScale(d.tissueSiteDetailId) !== undefined;
  }));
  let g = badges.enter().append("g").classed('tissue-badge', true);
  g.append('ellipse').attr('cx', bmap.xScale.range()[0] - bmap.xScale.bandwidth() / 2 - 10).attr('cy', d => bmap.yScale(d.tissueSiteDetailId) + bmap.yScale.bandwidth() / 2).attr('rx', 11) // Warning: hard-coded value
  .attr('ry', bmap.yScale.bandwidth() / 2).attr('fill', '#748797');
  g.append('text').text(d => d.rnaSeqAndGenotypeSampleCount).attr('x', bmap.xScale.range()[0] - bmap.xScale.bandwidth() / 2 - 17).attr('y', d => bmap.yScale(d.tissueSiteDetailId) + bmap.yScale.bandwidth() / 2 + 3).attr('fill', '#ffffff').style('font-size', "8px").attr('text-anchor', 'center');
}
/**
...
function findVariantsNearGeneStartEnd(gene, bmap) {
  ...
  const findLeftSideNearestNeighborVariant = site => {
    return variants.filter((d, i) => {
      ...
      if (pos === site) return true; // else find where the site is located
...
function renderGeneStartEndMarkers(bmap, bmapSvg, mini = false) {
  // rendering TSS
  if (mini) {
    let g = bmapSvg.select('#miniG').append('g').attr('id', 'miniSiteMarkers');
    g.append('line').attr('x1', bmap.xScaleMini(bmap.tss) + bmap.xScaleMini.bandwidth()).attr('x2', bmap.xScaleMini(bmap.tss) + bmap.xScaleMini.bandwidth()).attr('y1', 0).attr('y2', bmap.yScaleMini.range()[1]).style('stroke', '#94a8b8').style('stroke-width', 2);
    g.append('line').attr('x1', bmap.xScaleMini(bmap.tes) + bmap.xScaleMini.bandwidth()).attr('x2', bmap.xScaleMini(bmap.tes) + bmap.xScaleMini.bandwidth()).attr('y1', 0).attr('y2', bmap.yScaleMini.range()[1]).style('stroke', '#748797').style('stroke-width', 2);
  } else {
    bmapSvg.select('#siteMarkers').remove(); // clear previously rendered markers

    let g = bmapSvg.select('#focusG').append('g').attr('id', 'siteMarkers');

    if (bmap.tss && bmap.xScale(bmap.tss)) {
      g.append('line').attr('x1', bmap.xScale(bmap.tss) + bmap.xScale.bandwidth()).attr('x2', bmap.xScale(bmap.tss) + bmap.xScale.bandwidth()).attr('y1', 0).attr('y2', bmap.yScale.range()[1]).style('stroke', '#94a8b8').style('stroke-width', 2);
      g.append('text').text('TSS').attr('x', bmap.xScale(bmap.tss)).attr('y', -5).attr('text-anchor', 'center').style('font-size', "12px");
    }

    if (bmap.tes && bmap.xScale(bmap.tes)) {
      g.append('line').attr('x1', bmap.xScale(bmap.tes) + bmap.xScale.bandwidth()).attr('x2', bmap.xScale(bmap.tes) + bmap.xScale.bandwidth()).attr('y1', 0).attr('y2', bmap.yScale.range()[1]).style('stroke', '#748797').style('stroke-width', 2);
      g.append('text').text('TES').attr('x', bmap.xScale(bmap.tes)).attr('y', -5).attr('text-anchor', 'center').style('font-size', "12px");
    }
  }
}
/**
...
function buildVariantLookupTables(bmap) {
  bmap.rsLookUp = {};
  bmap.varLookUp = {};
  nest().key(d => d.x).entries(bmap.data).forEach(d => {
    let v = d.values[0];
...
function renderTssDistanceTrack(gene, bmap, bmapSvg) {
  let tss = gene.strand == '+' ? gene.start : gene.end; // color scale for the TSS Distance
  ...
  let colorScale = threshold().domain(domain).range(range);
  bmapSvg.select('#tssDistG').remove(); // clear any previously rendered SVG DOM objects.

  let g = bmapSvg.select('#focusG').append('g').attr('id', 'tssDistG');
  g.selectAll('.track').data(bmap.xScale.domain()).enter().append('rect').classed('track', true).attr('x', d => bmap.xScale(d)).attr('y', bmap.yScale.range()[1]).attr('width', bmap.xScale.bandwidth()).attr('height', bmap.yScale.bandwidth()).attr('fill', d => {
    let dist = Math.abs(parseFloat(d.split('_')[1]) - tss);
    return colorScale(dist);
  }).attr('stroke', d => bmap.variantsInExons[d] ? '#239db8' : '#cacaca').attr('stroke-width', d => bmap.variantsInExons[d] ? '2px' : '1px').on('mouseover', function (d) {
    let dist = Math.abs(parseFloat(d.split('_')[1]) - tss);
  ...
  g.append("text").text("TSS Proximity").attr("x", bmap.xScale.range()[0]).attr("y", bmap.yScale.range()[1] + bmap.yScale.bandwidth()).attr("text-anchor", "end").style("font-size", "8px");
}
/**
...
function renderBmapFilters(id, infoId, modalId, bmap, bmapSvg, tissueSiteTable) {
  ...
  let tiDiv = $('<div/>').attr('class', 'col-xs-12 col-sm-6 col-lg-1').css('padding-top', '4px').css('margin', '1px').css('font-size', '12px').appendTo($(`#${id}`));
  $('<span/>').attr('data-toggle', 'modal').attr('data-target', `#${modalId}`) // bMap-modal must be defined on the html
  .css('margin-left', '2px').css('padding-top', '2px').css('color', '#0868ac').css('cursor', 'pointer').html('<i class="fas fa-filter"></i>Filter Tissues<br/>').appendTo(tiDiv); ////// end adding custom DOMs
  ...
  if ($(`#${modalId}`).find(":input").length == 0) {
    ...
    bmap.yScale.domain().forEach(y => {
      // create a menu item for each tissue
      $('<input/>').attr('value', y).attr('type', 'checkbox').prop('checked', true).appendTo(modalBody);
      $('<label/>').css('font-size', '12px').css('margin-left', '2px').html(tissueSiteTable[y].tissueSiteDetail).appendTo(modalBody);
      $('<br/>').appendTo(modalBody);
    });
  } // Add all other eQTL filter panels


  panelBuilder(panels, id); // -- add the RS ID option here

  let rsDiv = $('<div/>').attr('class', 'col-xs-12 col-sm-6 col-lg-1').css('padding-top', '2px').css('margin', '1px').css('font-size', '12px').appendTo($(`#${id}`));
  let radioButton = $('<input/>').attr('id', 'rsSwitch').attr('type', 'checkbox').css('margin-left', '10px').appendTo(rsDiv);
  $('<label/>').css('margin-left', '2px').css('padding-top', '2px').css('font-size', '11px').html('Use RS ID').appendTo(rsDiv); ////// define the filter events
  ...
  miniG.selectAll('.mini-marker').data(bmap.xScaleMini.domain()).enter().append('rect').classed('mini-marker', true).attr('x', d => bmap.xScaleMini(d)).attr('y', bmap.yScaleMini.range()[1]).attr('width', bmap.xScaleMini.bandwidth()).attr('height', bmap.yScaleMini.bandwidth());
  $('#varLocator').keyup(e => {
    let v = $('#varLocator').val();

    if (v.length > 3) {
      const regex = new RegExp(v);
      focusG.selectAll('.bubble-map-xlabel').classed('query', d => {
        var bool = regex.test(d) || regex.test(bmap.rsLookUp[d]) || regex.test(bmap.varLookUp[d]);
        return bool;
      });
      miniG.selectAll('.mini-marker').classed('highlighted', d => {
        return regex.test(d) || regex.test(bmap.rsLookUp[d]) || regex.test(bmap.varLookUp[d]);
      });
    } else {
      focusG.selectAll('.bubble-map-xlabel').classed('query', false);
      miniG.selectAll('.mini-marker').classed('highlighted', false);
    }
  }); // rsId

  $('#rsSwitch').change(() => {
    if ($('#rsSwitch').is(':checked')) {
      focusG.selectAll('.bubble-map-xlabel').text(d => bmap.rsLookUp[d]);
    } else {
      focusG.selectAll('.bubble-map-xlabel').text(d => bmap.varLookUp[d]);
    }
  });
}
/**
...
function renderLDFilters(id, ldMap, ldCanvas, ldG, ldConfig) {
  ...
  panelBuilder(panels, id); // define the filter events:
...
function panelBuilder(panels, id) {
  panels.forEach((p, i) => {
    if ($(`#${p.id}`).length == 0) {
      // if it doesn't already exist in HTML document, then create it
      let div = $('<div/>').attr('id', p.id).attr('class', p.class).css('font-size', p.fontSize).css('margin', '1px').css('padding-top', '2px') // .css("white-space", "nowrap")
      .appendTo($(`#${id}`));
      div.addClass(p.class); // add the search box

      $('<label/>').css('font-weight', 'normal').html(p.search.label).appendTo(div);
      let input = $('<input/>').attr('id', p.search.id).attr('value', p.search.value).attr('size', p.search.size).attr('placeholder', p.search.placeholder).css('margin-left', '2px').appendTo(div); // add the slider if defined

      if (p.slider !== undefined) {
        $('<input/>').attr('id', p.slider.id).attr('value', p.slider.value).attr('type', p.slider.type).attr('min', p.slider.min).attr('max', p.slider.max).attr('step', p.slider.step).css('margin-left', '0px').css('width', '100px').appendTo(div);
      }
...
function addBubbleClickEvent(bmap, bmapSvg, par) {
  ...
  bmapSvg.selectAll('.bubble-map-cell').on("click", d => {
    ...
    let plot = $('<div/>').attr('class', 'bMap-dialog').css('float', 'left').css('margin', '20px').appendTo($(`#bMap-content`)); // add a header section

    let head = $('<div/>').appendTo(plot); // add a window-close icon

    $('<i/>').attr('class', 'fa fa-window-close').css('margin-right', '2px').click(function () {
      plot.remove();
    }).appendTo(head);
    $('<span/>').attr('class', 'title').html(`${d.displayX}<br/>${d.displayY}`).appendTo(head); // add the violin plot
    ...
    render$1(vConfig, d.gencodeId, d.variantId, d.tissueSiteDetailId, d.displayY, bmap.urls);
  });
}
/**
...
function createDialog(parentDivId, dialogDivId, title) {
  ...
  let dialog = $('<div/>').attr('id', dialogDivId).attr('title', title).appendTo(parent);
  let clearDiv = $('<div/>') // .attr('class', 'bMap-clear')
  .html("Clear All").appendTo(dialog);
  let contentDiv = $('<div/>').attr('id', 'bMap-content') // .attr('class', 'bMap-content')
  .appendTo(dialog);
  dialog.dialog({
    title: title,
    autoOpen: false
...
function _visualize(gene, variant, mainId, input, info) {
  ...
  if ($(`#${id.tooltip}`).length == 0) $('<div/>').attr("id", id.tooltip).appendTo($('body')); // clear previously rendered plot if any

  select(`#${id.main}`).selectAll("*").remove(); // build the dom elements
  ...
  let dom = select(`#${id.chart}`).append("svg").attr("width", width).attr("height", height).attr("id", id.svg).append("g").attr("transform", `translate(${margin.left}, ${margin.top})`); // add violin title

  dom.append("text").classed("ed-section-title", true).text(`${gene.geneSymbol} (${gene.gencodeId}) and ${variant.snpId || ""} (${variant.variantId})`).attr("x", 0).attr("y", -margin.top + 16); // render the violin

  let violin = new GroupedViolin(input, info);
  const tooltip = violin.createTooltip(id.tooltip);
  const toolbar = violin.createToolbar(id.toolbar, tooltip);
  toolbar.createDownloadSvgButton(id.buttons.save, id.svg, `${id.main}-save.svg`, id.clone);
  violin.render(dom, innerWidth, innerHeight, 0.3, undefined, [], "Norm. Expression", false, 0, true, 0, false, true, false, true, false, false);
...
function _submit(tissueGroups, dashboardId, menuId, pairId, submitId, formId, messageBoxId, urls = _getGTExUrls(), max = 30) {
  return function () {
    ...
    $(`#${dashboardId}`).html(''); ////// validate tissue inputs and convert them to tissue IDs //////
    ...
    pairs.forEach(function (pair, i) {
      ...
      let vid = pair.split(',')[1],
          gid = pair.split(',')[0]; // retrieve gene and variant info from the web service

      const geneUrl = urls.geneId + gid;
      const variantUrl = vid.toLowerCase().startsWith('rs') ? urls.snp + vid : urls.variantId + vid;
      Promise.all([json(geneUrl), json(variantUrl)]).then(function (args) {
        ...
        _renderEqtlPlot(tissueDict, dashboardId, gene, variant, queryTissueIds, i, urls); // hide the search form after the eQTL violin plots are reported
...
function _renderEqtlPlot(tissueDict, dashboardId, gene, variant, tissues, i, urls = getGtexUrls()) {
  ...
  $(`#${dashboardId}`).append(`<div id="${id}" class="col-sm-12"></div>`); // parse the genotypes from the variant ID

  let ref = variant.variantId.split(/_/)[2];
  let alt = variant.variantId.split(/_/)[3];
  const het = ref + alt;
  ref = ref + ref;
  alt = alt + alt; // d3-queue https://github.com/d3/d3-queue

  let promises = []; // queue up all tissue IDs

  tissues.forEach(tId => {
    let urlRoot = urls['dyneqtl']; // let url = `${urlRoot}?snp_id=${variant.variantId}&gene_id=${gene.gencodeId}&tissue=${tId}`; // use variant ID, gencode ID and tissue ID to query the dyneqtl
...
function _apiCall(url, tissue) {
  ...
  return new Promise(function (resolve, reject) {
    json(url).then(function (request) {
      resolve(request);
    }).catch(function (err) {
...
exports.BatchGeneExpression = BatchGeneExpression;
exports.EqtlDashboard = EqtlDashboard;
exports.EqtlViolinPlot = EqtlViolinPlot;
exports.GeneEqtlVisualizer = GeneEqtlVisualizer;
exports.GeneExpressionBoxplot = GeneExpressionBoxplot;
