...
function render(type, geneId, rootId, urls = getGtexUrls()) {
  ...
  .then(function (data) {
    ...
    const promises = [json(urls.tissue, {
      credentials: 'include'
...
function _addToolbar(vplot, tooltip, ids, urls) {
  ...
  $(`<button class="btn btn-default fa fa-caret-down" id="${ids.buttons.descAlphaSort}"></button>`).appendTo(alphaSortButtonGroup); // sort options -- median sorts
  ...
  $(`<button class="btn btn-default fa fa-caret-down" id="${ids.buttons.descSort}"></button>`).appendTo(numSortButtonGroup); // outlier display options
...
function searchById(heatmapRootId, violinRootId, glist, tlist = undefined, urls = getGtexUrls(), filterGenes = undefined, callback = undefined, qTissue = undefined) {
  ...
  Promise.all([json(urls.tissue, {
    credentials: 'include'
  }), json(urls.geneId + glist.join(","), {
    credentials: 'include'
  })]).then(function (args) {
...
function render$2(par, geneId, urls = getGtexUrls()) {
  ...
  json(urls.geneId + geneId, {
    credentials: 'include'
  }) // query the gene by geneId which could be gene name or gencode ID with or withour versioning
  .then(data => {
    let gene = parseGenes(data, true, geneId); // report the gene info

    $(`#${par.divGeneInfo}`).empty();
    $("<span/>").html(`<span>${gene.geneSymbol} (${gene.gencodeId}), ${gene.chromosome}:${gene.start} - ${gene.end} (${gene.strand}), ${gene.description}`).appendTo($(`#${par.divGeneInfo}`));
    let promises = [json(urls.tissue, {
      credentials: 'include'
...
function renderBmapFilters(id, infoId, modalId, bmap, bmapSvg, tissueSiteTable) {
  ...
  if ($(`#${modalId}`).find(":input").length == 0) {
    ...
    bmap.yScale.domain().forEach(y => {
      ...
      let x = $('<label/>');
      $('<input/>').attr('value', y).attr('type', 'checkbox').prop('checked', true).appendTo(x);
      $('<span/>').css('font-size', '12px').css('margin-left', '2px').html(tissueSiteTable[y].tissueSiteDetail).appendTo(x);
